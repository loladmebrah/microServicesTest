...
var port = process.env.PORT_SERVICE2 || 690;
...
var auxiliarUrl = process.env.SERVICE1_URL || "http://service1:420";
...
app.get("/base2", function (req, res) {
  regularSafeRequest(
    baseUrl + "/vehicles/GetAllManufacturers?format=json&page=1",
    (err, data) => {
...
app.get("/magic2", function (req, res) {
  ...
  regularUnsafeRequest(finalUrl, (err, data) => {
    if (err) return res.status(400).json({ msg: err });
      ...
      return res
        .status(200)
        .json({ msg: "from api 1 with love", response: data });
    }
  });
});
