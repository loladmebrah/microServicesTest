...
var auxiliarUrl = 'http://localhost:690/';
...
app.get('/base1', function(req, res){
    regularSafeRequest(baseUrl+'/vehicles/DecodeVinValues/5UXWX7C5*BA?format=json&modelyear=2020',(err, data)=>{
...
app.get('/magic1', function(req, res){
    ...
    regularUnsafeRequest(finalUrl,(err, data)=>{
        if(err) return res.status(400).json({msg:err});
            ...
            return res.status(200).json({msg: "from api 2 with love", response: data});
        }
    })
});
